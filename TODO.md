...
- [ ] Reduce Firestore reads/writes. Check Cloud functions, if they are optimized and do not read/write more documents than needed. For example, when adding a pour, we currently read the whole session document to get the current `last_volumes_ml` map, then write the new pour and update the map with one transaction. This is good for consistency, but it means every pour causes a read of the whole session document (which can be large if there are many pours). Consider if we can optimize this by denormalizing last volume into each participant's subcollection or using a separate document for tracking last volumes. Also review Cloud Functions for any inefficient queries or updates, especially those triggered on every pour or session update. Also check beer/brewery search queries for optimization (e.g. indexing, limiting results, longer wait for user input). Eg. notifications related to keg nearly empty or done should not read all participants if we can filter by those who have notifications enabled and can be triggered from user application when detecting the threshold crossing, instead of relying on Cloud Function trigger on session update.
